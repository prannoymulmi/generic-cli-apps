...
- The user runs the CLI for a month that already has entries logged against the chosen project/task on some weekdays: those rows appear in the preview marked "already logged", are excluded from the submission, and cannot be re-included from inside the CLI (per FR-012).
...
- **FR-005**: The CLI MUST compute the set of Mon–Fri dates in the chosen month and generate one planned 8-hour entry per date, excluding Saturdays and Sundays.
...
- **FR-008**: The user MUST be able to edit the preview before submission. Per row, the user can (a) change the hours value from the default 8 to any non-negative number and (b) toggle the row between "included" and "skipped". Per-row description text is NOT editable in v1 — all entries share a single fixed description (see Assumptions).
...
- **FR-011**: The CLI MUST report the outcome of submission clearly, indicating either total success or — if the bulk call fails — that no entries were created.
- **FR-012**: If the chosen month already has time entries logged against the chosen project/task on some weekdays, the CLI MUST mark those rows as "already logged" in the preview, exclude them from the submission batch by default, and prevent the user from toggling them back to "included" (to avoid duplicate entries). Adjusting an existing day's hours must be done in the Moco web UI.
...
- **Submission Result**: The outcome reported back from Moco after the bulk request — overall success/failure and, on failure, enough information for the user to understand what to do next.
...
- "8 hours per weekday" is the default plan for the current need; users may edit this in the preview if FR-008 is resolved to allow it.
