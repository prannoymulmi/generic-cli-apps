...
- [X] T008 [P] [US2] Create `tests/test_styling.py` covering `is_color_enabled()` across all four trigger combinations: no env / `NO_COLOR=1` / `sys.stdout.isatty() == False` (monkeypatch) / `TERM=dumb`. Use `monkeypatch.setenv` and `monkeypatch.setattr(sys, 'stdout', ...)` so the test is hermetic.
- [X] T009 [P] [US2] In `tests/test_styling.py`, add tests for `build_style()`: returns `None` when colour disabled; returns a `prompt_toolkit.styles.Style` instance when colour enabled (assert via `isinstance` only — the exact RGB values are tunable per the contract).
...
- [X] T011 [US2] Create `src/moco_filler/styling.py` with `is_color_enabled() -> bool` implementing the three-branch predicate from `research.md` §3 — returns `False` on non-empty `NO_COLOR`, non-TTY stdout, or `TERM=="dumb"`, `True` otherwise. Imports only `os` and `sys`.
...
- [X] T014 [US2] In `src/moco_filler/styling.py`, add `format_styled_header() -> list[tuple[str, str]] | str` mirroring `preview.format_header()` but wrapped in `("class:row.header", text)` when colour is enabled.
- [X] T015 [US2] In `src/moco_filler/preview.py`, update `_build_choices(entries)` so each data-row `Choice.title` becomes `format_styled_row(entry)` (the new helper) and the leading `Separator` uses `format_styled_header()`. Keep the monochrome path identical to US1's output.
...
- [X] T017 [US2] In `src/moco_filler/cli.py`, replace the project picker's choice titles with the styled-row equivalent of the project name (one styled `Choice` per project) and do the same for the task picker, so all selection screens share the colour vocabulary defined in `styling.py` (FR-008a).
